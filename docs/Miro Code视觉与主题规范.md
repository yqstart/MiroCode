...
|---|---|---|
| `--bg-app` | `#0F0F12` | 应用底 |
| `--bg-header` | `#1A1A20` | macOS Overlay 标题栏底色（仅最顶系统栏） |
...
| `--bg-header` | `#E8EAEF` | macOS Overlay 标题栏底色（仅最顶系统栏，不含项目行） |
...
- 编辑器、快捷键、关于（不含 AI 对话面板 / Agent / MCP） 
- 语言服务、快捷键、代码索引、安全、系统  
...
3. （远期预留）AI 行内补全 — UI 可占位，能力见排期远期；非独立面板  
...
3. 主题四宫格布局存在，Dawn / Miro Dark 可真实切换  
